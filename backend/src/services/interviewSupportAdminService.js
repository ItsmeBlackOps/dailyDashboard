...
class InterviewSupportAdminService {
  ...
  async getLogs(date) {
    ...
    const PROCESSED_PHASES = new Set([
      ...
      // Fireflies — successful bot lifecycle counts as processing.
      'FIREFLIES_INVITE_SUCCESS', 'FIREFLIES_VERIFY_SUCCESS', 'FIREFLIES_MAIN_JOINED',
    ]);
    const FAILED_PHASES = new Set([
      ...
      'FIREFLIES_INVITE_FAILED', 'FIREFLIES_VERIFY_FAILED', 'FIREFLIES_MAIN_FAILED',
    ]);
    const PENDING_PHASES = new Set([
      'AUTO_ASSIGN_QUEUED', 'AUTO_ASSIGN_STARTED', 'SEARCH_RETRY_WAIT', 'SEARCH_STARTED',
      'FIREFLIES_INVITE_ATTEMPT', 'FIREFLIES_MAIN_RETRY', 'FIREFLIES_VERIFY_NOT_IN_MEETING',
    ]);
