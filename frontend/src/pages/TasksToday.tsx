    ...
    async (task: Task) => {
      ...
      try {
        ...
        if (data?.pending) {
          ...
          // next list refresh. No duplicate creation.
          return;
        }

        const joinUrl: string = data?.meetingLink || data?.joinUrl || data?.joinWebUrl || '';
        if (joinUrl) {
          setTasks((prev) =>
            prev.map((item) =>
              item._id === task._id ? { ...item, meetingLink: joinUrl, joinUrl, joinWebUrl: joinUrl } : item
            )
          );
        }
